...
## 概要や注意点など
- 動画のコピータスクや圧縮タスクは書いてない（重い為）ので、必要があれば設定する。各スクリプトもタスク内容に合わせて更新する。
...
- .pipe(cache('html'))の部分のコメントアウトを外す（処理を通す）と、変更を加えたファイルだけがコンパイルされるので、コンパイルがとても早くなる。
...
### Javascript関連
- JSをgulpにキャッシュさせて、更新したファイルのみコンパイル
- 複数のJSファイルを一つに統合
...
### 上記ではそれぞれタスクを書いていますが、初期設定では以下のようになっています。
- 本番環境以外では基本的にHTML・CSS・JSは圧縮しない。
- 本番環境以外ではCSS・JSのインライン化はしない。インライン化させると、CSSなどのファイル変更時にブラウザリロードさせる機能と併用出来ない為。
- 検証環境がある場合、ogタグの設定は検証環境のURLを参照させる。
- 本番環境以外ではhtmlのhead内に、metaタグnoindexを設置（Googleにインデックスさせないことにより、検索しても出てこないようにする設定）
...
- pages以下
  実際に出力されるファイルを定義します。ページを増やすごとにpugファイルを増やします。このディレクトリ以下はディレクトリ構造もそのまま保持して出力されます。
...
- pages
  実際に出力されるファイル群。上記までの組み合わせで実現出来ないスタイルを当てていく。
...
2. 同じく`src/pug/config.pug`に増やした分のファイルの設定を`var settings`内に例を見ながら記述。
